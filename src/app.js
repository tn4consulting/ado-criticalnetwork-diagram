...
function init(data){
  ...
  // Fits the current view's full width into the visible canvas — used as
  // the responsive default (initial load + window resize) whenever the
  // user hasn't manually overridden zoom. Never upscales past 100% native.
  function fitZoomToView(){
    const view = activeZoomableView();
    if(!view || !view.w) return 100;
    const availW = view.wrap.clientWidth - 24;
    if(availW <= 0) return 100;
    return Math.min(100, Math.max(ZOOM_MIN, Math.floor((availW / view.w) * 100)));
  }
